"""Train the SwavBBoxModel on OUHANDS."""
...
from swav_bbox_model import SwavBBoxModel

def evaluate_model_swav(
    model: nn.Module,
    loader: DataLoader,
    criterion: nn.Module,
    device: torch.device,
...
def classification_metrics_swav(
    model: nn.Module,
    loader: DataLoader,
    device: torch.device,
    use_amp: bool,
...
def build_swav_bbox_model(config: ExperimentConfig, device: torch.device) -> tuple[nn.Module, dict[str, object]]:
    model = SwavBBoxModel(
    ...
    # Define transforms for SwAV (ResNet-like)
    # We can use standard ImageNet transforms or similar to DINO
    # Since we are cropping to bbox, the input to the network will be 224x224
    # But the dataset returns the full image.
    
    # We'll use the same transforms as DINO for consistency in data augmentation
    # but we need to be careful about resizing.
    # The dataset will return the full image (e.g. 224x224 or larger).
    # Our model will crop it.
    
    # Let's use a standard config from a timm resnet50 to get transforms
...
def train_one_epoch_swav(
    ...
    for batch in loader:
        ...
        images, labels, bboxes, masks, paths = _unpack_batch(
            batch, 
            use_bounding_box=True, 
            use_segmentation=False, # We don't need segmentation for this model
            return_paths=True
        )
...
def run_training(args: argparse.Namespace) -> None:
    ...
    # Disable segmentation for this specific training script
    ...
    model, transforms = build_swav_bbox_model(config, device)
    ...
    print("Training SwavBBoxModel")
    for epoch in range(1, config.training.epochs + 1):
        train_loss, train_acc = train_one_epoch_swav(
            model,
            train_loader,
            optimizer,
            criterion,
        ...
        # Evaluate
        val_loss, val_acc = evaluate_model_swav(
            model,
            val_loader,
            criterion,
            device,
    ...
    if best_state is not None:
        ...
        ckpt_path = checkpoint_dir / "swav_bbox_best.pt"
        torch.save(best_state, ckpt_path)
        print(f"Saved best checkpoint (val_acc={best_acc:.4f}, val_loss={best_loss:.4f}) to {ckpt_path}")

    # Test
    test_metrics = classification_metrics_swav(model, test_loader, device, use_amp)
    print("Test metrics: " + " ".join(f"{k}={v:.4f}" for k, v in test_metrics.items()))


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SwavBBoxModel on OUHANDS")
