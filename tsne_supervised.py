"""Generate a t-SNE visualization for a supervised ViT checkpoint on OUHANDS."""
...
def build_supervised_eval_model(
    config: ExperimentConfig,
    device: torch.device,
    arch: str,
) -> tuple[nn.Module, Dict[str, object]]:
    """Instantiate the supervised ViT architecture used by the baseline script."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a t-SNE plot for a supervised ViT checkpoint")
    parser.add_argument("checkpoint", type=Path, help="Path to the checkpoint to visualize")
    parser.add_argument(
        "--arch",
        type=str,
        default="vit_base_patch16_224",
        help="timm model name matching the supervised checkpoint architecture",
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/tsne_supervised.png"),
...
def main() -> None:
    ...
    model, transforms = build_supervised_eval_model(config, device, args.arch)
